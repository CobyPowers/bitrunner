...
func _ready() -> void:
	...
	if segments.size() <= 0:
		...
		get_tree().quit(1)
	if get_parent() == game_manager.get_parent():
		spawn_random_segment(transform)
		queue_free()
	else:
...
func spawn_random_segment(new_transform: Transform2D = next_segment_anchor.transform):
	var segment = get_random_segment().instantiate()
	segment.transform = new_transform
	segment.segments = segments
	get_tree().root.call_deferred("add_child", segment)
...
func _on_entered_trigger_body_entered(body: Node2D) -> void:
	...
	entered_sfx.play()
	game_manager.advance_lava(global_position, 10)
